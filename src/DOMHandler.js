...
const homeListener = function (event) {
  ...
  tasks = tasks.filter((task) => {
    ...
    return isSameMonth(task.dueDate, new Date());
  });
...
const projectListener = function (event) {
  document.querySelector('.main-subtitle').textContent = event.target.textContent;
  const { tasks } = Project.projects[parseInt(event.target.id)];
  const mainTasks = document.querySelector('.show-tasks');
...
const newProjectHandler = function (event) {
  ...
  yes.addEventListener('click', (event2) => {
    event2.preventDefault();
    new Project(event2.target.parentNode.projectName.value);
...
const newTaskHandler = function (event) {
  ...
  button.addEventListener('click', (event2) => {
    const taskForm = event2.target.parentNode;
    event2.preventDefault();
...
export {
  addHomeListeners, addProjectListeners, pageLoad, addButtonHandlers,
};
